...
module.exports = function(User) {
  ...
  User.login = function(credentials, include, fn) {
    ...
    if (realmRequired && !credentials.realm) {
    ...
    if (!credentials.email && !credentials.mobile) {
      var err2 = new Error(g.f('{{mobile}} or {{email}} is required'));
      err2.statusCode = 400;
      err2.code = 'MOBILE_EMAIL_REQUIRED';
      fn(err2);
      return fn.promise;
    }
    if (!credentials.password) {
      var err3 = new Error(g.f('{{password}} is required'));
      err3.statusCode = 400;
      err3.code = 'PASSWORD_REQUIRED';
      fn(err3);
      return fn.promise;
    }
      ...
      if (err) {
        ...
        user.hasPassword(credentials.password, function(err, isMatch) {
          if (err) {
            ...
            if (self.settings.emailVerificationRequired && !user.emailVerified) {
            ...
            else if (self.settings.mobileVerificationRequired && !user.mobileVerified) {
              // Fail to log in if email verification is not done yet
              debug('User mobile has not been verified');
              err = new Error(g.f('login failed as the mobile has not been verified'));
              err.statusCode = 401;
              err.code = 'LOGIN_FAILED_MOBILE_NOT_VERIFIED';
              err.details = {
                userId: user.id,
              };
              fn(err);
            }
            else if (user.realm == 'kitchen' && user.adminVerifiedStatus == 'pending') {
              err = new Error(g.f('login failed as the account has not been verfied'));
              err.statusCode = 401;
              err.code = 'LOGIN_FAILED_ACCOUNT_NOT_VERIFIED';
              err.details = {
                userId: user.id
              };
              fn(err);
              ...
              err = new Error(g.f('login failed as the account has not been rejected by admin'));
              err.statusCode = 401;
              err.code = 'LOGIN_FAILED_ACCOUNT_REJECTED';
              err.details = {
                userId: user.id
              };
              fn(err);
            ...
            debug('The password is invalid for user %s', credentials.email || credentials.mobile);
            var passError = new Error(g.f('Incorrect password'));
            passError.statusCode = 401;
            passError.code = 'INCORRECT_PASSWORD';
            fn(passError);
          }
        });
      } else {
        debug('No matching record is found for user %s', credentials.email || credentials.mobile);
        fn(defaultError);
      }
    });
    return fn.promise;
  ...
  *    });
  ...
  User.prototype.verify = function(verifyOptions, options, cb) {
    ...
    urlPath = verifyOptions.apiUrl || urlPath;
    // verifyOptions.verifyHref = config.redirectApiUrl +
    //   urlPath +
    //   '?' + qs.stringify({
    ...
    function sendEmail(user) {
      ...
      function setHtmlContentAndSend(err, html) {
        ...
        verifyOptions.from = "dollygarg.zed@gmail.com";
        ...
        if (user && user.verificationToken === token) {
          ...
          if (user) {
            ...
            if(user.verificationToken) {
             ...
             err.name = "Token error";
              err.message ="You have been already verified your account."
            }
          } else {
            ...
            err.name="";
            err.message ="You are not registered in application."
          }
          fn(err);
        }
      }
